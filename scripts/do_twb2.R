...
teom_locs <- pair_teoms(filter(teom_locs, deployment!="Olancha", 
                               deployment!="North Beach"))
...
teom_data <- inner_join(teom_data, select(teom_locs, deployment.id, deployment, 
                                          dca.group, position),
                        by="deployment.id")
...
events_summary <- clean_events %>% group_by(dca.group, dir) %>% 
  summarize(mean.pm10.uw = mean(pm10.uw), mean.pm10.dw = mean(pm10.dw), 
            mean.pm10.delta = mean(pm10.delta), 
            n = length(pm10.uw))

# distributions of pm10 deltas for north and south dca group events (central dca 
# group does not have enough events to make useful conculsions) 
p_nn <- ggplot(filter(clean_events, dca.group=="north", dir=="N"), 
               aes(x=pm10.delta)) + 
        geom_density()
p_ns <- ggplot(filter(clean_events, dca.group=="north", dir=="S"), 
               aes(x=pm10.delta)) +
        geom_density()
p_sn <- ggplot(filter(clean_events, dca.group=="south", dir=="N"), 
               aes(x=pm10.delta)) +
        geom_density()
p_ss <- ggplot(filter(clean_events, dca.group=="south", dir=="S"), 
               aes(x=pm10.delta)) +
        geom_density()

# plots of relevant DCA groups with associated teoms and upwind angles
p_north <- plot_owens_borders(dcas=north_group) +
           geom_point(data=filter(teom_locs, dca.group=="north"), 
                      mapping=aes(x=x, y=y)) +
           geom_label(data=filter(teom_locs, dca.group=="north"), 
                      mapping=aes(x=x, y=y, label=deployment)) 
p_central <- plot_owens_borders(dcas=central_group) +
           geom_point(data=filter(teom_locs, dca.group=="central"), 
                      mapping=aes(x=x, y=y)) +
           geom_label(data=filter(teom_locs, dca.group=="central"), 
                      mapping=aes(x=x, y=y, label=deployment)) 
p_south <- plot_owens_borders(dcas=south_group) +
           geom_point(data=filter(teom_locs, dca.group=="south"), 
                      mapping=aes(x=x, y=y)) +
           geom_label(data=filter(teom_locs, dca.group=="south"), 
                      mapping=aes(x=x, y=y, label=deployment)) 

write.csv(events_summary, 
          file="./output/twb2_paired_teoms_DEC_events_summary.csv")
